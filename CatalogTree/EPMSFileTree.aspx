<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="EPMSFileTree.aspx.cs"
    Inherits="Zhongsoft.Portal.DCMS.List.EPMSFileTree" %>
    
<%@ Register Src="../Metro/Controls/MetroHeader.ascx" TagName="MetroHeader" TagPrefix="uc2" %>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
    <title>设计人员工作平台</title>
...
<body>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $.post("EPMSFileTree.aspx?organizationID=" + '<%=OrganizationID %>' + "&rollID=" + '<%=RollID %>' + "&specialtyID=" + '<%=SpecialtyOfProjectEntityID %>' + "&externalFileID=" + "&mutualFileID=",
                       { asyfunc: "GetProject" },
                           function (res) {
                               ...
                               var node;
                               if ('<%=RollID %>' != '') {
                                   node = treeObj.getNodeByParam('id', '<%=RollID %>', null);
                                   treeObj.selectNode(node);
                                   treeObj.setting.callback.onClick = openUrl(node.url);
                               } else if ('<%=SpecialtyOfProjectEntityID %>' != '') {
                                   node = treeObj.getNodeByParam('id', '<%=SpecialtyOfProjectEntityID %>', null);
                                   treeObj.selectNode(node);
                                   treeObj.setting.callback.onClick = openUrl(node.url);
                               }
