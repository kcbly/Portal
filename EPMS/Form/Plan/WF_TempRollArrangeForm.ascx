...
<script>
    ...
    function getSelWBSFilter() {
        ...
        if (specialtyIDs == "") {
            ...
            //filter = { OrganizationID: organizationID, WBSType: '<%=(int)WBSMode.卷册管理 %>', ExeSpecialtyID: specialtyIDs, IsHavePlaned: 'false', WBSFormTypeSelect: formType, IsFilterSpe: "true" };
            filter = { OrganizationID: organizationID, WBSType: '<%=(int)WBSMode.卷册管理 %>', ExeSpecialtyID: specialtyIDs, IsHavePlaned: 'false', IsFilterSpe: "true" };
        }
        return filter;
    }
