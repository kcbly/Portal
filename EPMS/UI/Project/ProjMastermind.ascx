...
<%@ Register Src="~/EPMS/UI/Project/ProjectCommon.ascx" TagName="ProjectCommon" TagPrefix="uc6" %>
...
<%@ Register Src="~/EPMS/UI/WBS/WBSTreeAndList.ascx" TagName="WBSTreeAndList" TagPrefix="uc2" %>
<%@ Register Src="~/EPMS/UI/OBS/ProjectMgrUser.ascx" TagName="ProjectMgrUser" TagPrefix="uc5" %>
<%@ Register Src="~/EPMS/UI/plan/ProjectPlanBookCreate.ascx" TagName="ProjectPlanBookCreate" TagPrefix="uc6" %>
<%@ Register Src="../../UI/WBS/WBSTreeAndList.ascx" TagName="WBSTreeAndList" TagPrefix="uc7" %>
    ...
    <tr id="ProjectCommon">
        <td colspan="6">
            <table class="tz-table">
                <uc6:ProjectCommon ID="ucProjectCommon" runat="server" />
            </table>
        </td>
    </tr>
</table>
                ...
                <tr>
                    ...
                    <td class="td-m" colspan="5">
                        <zhongsoft:LightTextBox ID="tbResourceRequire" runat="server" TextMode="MultiLine"
                            MaxText="512" CssClass="kpms-textarea" Width="96%" EnableTheming="false" ActiveStatus="(23.编制项目策划、配置人员)"
                            Field="ResourceRequire" TableName="EPMS_ProjectPlanBook"></zhongsoft:LightTextBox>
                    </td>
                </tr>
                <uc5:ProjectMgrUser ID="ucProjectMgrUser" runat="server" />
    ...
    <tr id="projWBS">
        <td colspan="6">
            <table class="tz-table" style="width: 100%">
                ...
                <uc2:WBSTreeAndList ID="ucWBSTreeAndList" runat="server" OnWBSChanged="ReLoadWBSInfo" />
            </table>
        </td>
    </tr>
</table>
<table class="tz-table" width="100%">
    ...
    <tr id="planProjInfo">
        <td colspan="6">
            ...
            <uc4:IntegrateProjItemList ID="ucIntegrateProjItemList" runat="server" />
        </td>
    </tr>
</table>
<uc6:ProjectPlanBookCreate ID="ucProjectPlanbookCreate" runat="server" />
<!----项目计划书ID--->
    ...
    function projMastermind_Init() {
        ...
        <%--if ("<%=ActionType %>" == "<%=AccessLevel.Update %>") {
            ...
            if ("<%=PhaseCode %>" == "S") {
                ...
                //非施工图
                if ($actName == "修改计划" || $actName == "初始化WBS任务") {
                    $('[flag="wbsInfo"]').show();
                }
                else {
                    $('[flag="wbsInfo"]').hide();
        ...
        } else if ("<%=ActionType %>" == "<%=AccessLevel.Read %>") {
            //查看状态
            //查看状态显示
            $('[flag="wbsInfo"]').show();
        }--%>
