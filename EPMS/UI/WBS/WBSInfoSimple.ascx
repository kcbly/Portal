...
<%@ Register Src="../../UI/WBS/FileReceiveWBSList.ascx" TagName="FileReceiveWBSList"
    TagPrefix="uc2" %>
<%@ Register Src="../../UI/WBS/FileUpgradeList.ascx" TagName="FileUpgradeList" TagPrefix="uc3" %>
...
<tr style="display: none">
    ...
    <td class="td-l" flag="onlyCe" style="display: none">卷册类别
    </td>
...
<tr flag="parent">
    <td class="td-l">卷册名称<span class="req-star" id="spanRoll">*</span>
    </td>
    <td class="td-r">
...
<tr flag="recroll">
    ...
    <td colspan="3" class="td-m">
        <span id="Span2" runat="server" style="float: right" class="req-star" displaystatus="(2.*)"
            flag="rollPaper">请先保存，再添加卷册</span>
...
<tr flag="upgradememo">
    ...
    <td colspan="3" class="td-m" style="color: Red;">1.全部继承：升版卷册继承原版卷册的资料项：包括资料编号、名称等，提交状态为已提交，不继承具体资料文件，在资料台帐中查看流程时会跳转到继承的资料提交流程<br />
        2.部分继承：升版卷册继承原版卷册的资料项：包括资料编号、名称等，提交状态为未提交，需要单独走资料提交流程，不继承具体的资料文件<br />
        3.不 继 承：升版卷册不继承原版卷册的任何资料，需要单独走资料提交流程
    </td>
</tr>
...
<tr flag="form" style="display: none;">
    ...
    <td class="td-r">
        <a runat="server" id="aTask" title="点击查看卷册任务书" class="aStyle">查看</a>
        <asp:Label runat="server" ID="lbNoTaskMsg" Text="该卷册没有卷册任务书" CssClass="aStyle-red"
            Visible="false"></asp:Label>
    </td>
    <td class="td-l">成品校审单
    </td>
    <td class="td-r">
        <a runat="server" id="aVerify" title="点击查看成品校审单" class="aStyle">查看</a>
        <asp:Label runat="server" ID="lbNoVerifyMsg" Text="该卷册没有成品校审单" CssClass="aStyle-red"
            Visible="false"></asp:Label>
    </td>
</tr>
<tr>
...
<script type="text/javascript">
    ...
    function initWbsName(isChange) {
        ...
        if (action == 'batch') {
            ...
            $('[flag = "manufacturefile"]').hide();
            $('[flag = "affectedinterface"]').hide();
            if (wbsMode == '<%=(int)WBSMode.卷册管理 %>') {
                //卷册信息
                $('[flag = "roll"]').show();
                $('[flag = "name"]').each(function () {
                    $(this).text($(this).text().replace('任务', '卷册'));
                });
            ...
            //只有卷册、成品管理才显示检索号
            if (wbsMode == '<%=(int)WBSMode.卷册管理 %>' || wbsMode == '<%=(int)WBSMode.成品管理 %>') {
                $('[flag="serial"]').show();
                //$('[flag="noserial"]').hide();
            }
            else {
    ...
    function CompareDate(planDate) {
        ...
        if (planFinishDate != "" && thisDate != "") {
            if (new Date(thisDate.replaceAll('-', '/')) > new Date(planFinishDate.replaceAll('-', '/'))) {
                alert("卷册或提资计划完成日期不能超过阶段计划完成日期" + planFinishDate.replaceAll('/', '-').replaceAll('0:00:00','') + "，如需修改请联系设总修改项目计划");
                planDate.value = "";
            }
        }
    }
