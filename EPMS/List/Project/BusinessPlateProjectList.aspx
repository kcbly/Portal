...
<asp:Content ID="Content3" ContentPlaceHolderID="toolBtn" runat="server">
    <asp:LinkButton ID="lkbProjectStart" runat="server" EnableTheming="false" CssClass="btn-query"
        OnClientClick="return confirm('是否启动当前选中项目？');" OnClick="lkbProjectStart_Click"><span>启动项目</span></asp:LinkButton>
    <asp:Label runat="server" ID="lblPrompt" Text="沟选需要启动的项目，然后启动项目" CssClass="req-star"></asp:Label>
    <asp:LinkButton ID="lkbNewProject" runat="server" OnClientClick="return addNewProject()"
        EnableTheming="false" CssClass="btn-query"><span>新建子项</span></asp:LinkButton>
</asp:Content>
<asp:Content ID="Content4" ContentPlaceHolderID="listGrid" runat="server">
    <zhongsoft:LightPowerGridView Width="100%" runat="server" ID="gvList" AllowPaging="True"
        ...
        <Columns>
            ...
            <zhongsoft:LightTemplateField HeaderText="项目名称" ItemStyle-Width="200px">
                <ItemTemplate>
                    <a onclick="return viewItem('1','<%#Eval("OrganizationId") %>')" title='<%# Eval("ProjectName")%>'
                        style="color: Blue">
                        <%#Eval("ProjectName").ToString().Length > 25 ? Eval("ProjectName").ToString().Substring(0, 24) + "..." : Eval("ProjectName")%>
                    </a>
                </ItemTemplate>
            </zhongsoft:LightTemplateField>
            <zhongsoft:LightBoundField DataField="ManagerUserName" HeaderText="项目设总A" ItemStyle-Width="100px">
            </zhongsoft:LightBoundField>
            ...
            <zhongsoft:LightBoundField DataField="InnerCompleteDate" HeaderText="计划完成日期" HtmlEncode="false"
                DataFormatString="{0:yyyy-MM-dd}">
                <ItemStyle Width="60px" />
            </zhongsoft:LightBoundField>
            ...
            <zhongsoft:LightTemplateField HeaderText="计划工程师">
                <ItemTemplate>
                    <asp:Label ID="lbJhgcs" runat="server">
                    <span jhgcs='<%#Eval("OrganizationId") %>'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></asp:Label>
                </ItemTemplate>
            </zhongsoft:LightTemplateField>
            <zhongsoft:LightTemplateField HeaderText="质量工程师">
                <ItemTemplate>
    ...
    <script type="text/javascript">
        ...
        extendCheckGridRow("");
        function extendCheckGridRow(array) {
            var proIDs = array;
            if (proIDs !=null && proIDs.length ==1 ) {
                $("#<%=lkbProjectStart.ClientID %>").show();
                $("#<%=lkbNewProject.ClientID %>").hide();
            } else {
                $("#<%=lkbProjectStart.ClientID %>").hide();
                $("#<%=lkbNewProject.ClientID %>").hide();
            }
        }
